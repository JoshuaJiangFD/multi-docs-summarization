...
public class DocProcessor implements IDocProcessor {
    ...
    private final List<String> ADOPTEDWORDATTRs=Lists.newArrayList("n","t","v","a","f");
     ...
     * All the Id field in document and sentence are empty.
     * 
     * @param content
     */
    public MdsDocument genDocument(String content) {
        ...
        List<String> sentenceStrs = this.splitContent(content);
            ...
            for (String str : wordStrs) {
                ...
                if (pos > 0) {
                    ...
                    if (STOPWORDS.contains(str1))
                        continue;
                    Boolean attriPassed=Iterables.any(ADOPTEDWORDATTRs, new Predicate<String>(){
                        @Override
